...
def to_npy(x):
    ...
    # return x.cpu().data.numpy() if device == 'cpu' else x.detach().numpy()
...
class Embed(nn.Module):
    def __init__(self, ex, emb_size, loc_max, embed_layers):
        ...
        _, _, _, _, _, self.emb_su, self.emb_sl, self.emb_tu, self.emb_tl = embed_layers
    ...
    def forward(self, traj_loc, mat2, vec, traj_len):
        ...
        # print(vec.shape)
        # unsqueeze()用来增加维度，-1表示在最后一个位置扩充一个维度，例如形状为(1,50)->（1,50,1）
        # expand只能对维度值为1的维度进行扩展，这里表示第三个维度的纬度值扩展到self.loc_max
        ...
        for i in range(mask.shape[0]):  # N
            ...
            # print(delta_s[i, :traj_len[i]])
            delta_s[i, :traj_len[i]] = torch.index_select(mat2, 0, (traj_loc[i]-1)[:traj_len[i]])
            # print(torch.index_select(mat2, 0, (traj_loc[i]-1)[:traj_len[i]])
        # pdb.set_trace()
...
class MultiEmbed(nn.Module):
    def __init__(self, ex, emb_size, embed_layers):
        ...
        self.emb_t, self.emb_l, self.emb_u, self.emb_m, self.emb_c,\
    ...
    def forward(self, traj, mat, traj_len):
        ...
        month = self.emb_m(traj[:, :, 3])
        category = self.emb_c(traj[:, :, 4])
        # print(traj[:, :, 4])
        joint = time + loc + user + month + category # (N, M, embed)
