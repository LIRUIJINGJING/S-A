...
class Trainer:
    ...
    def train(self):
        ...
        for t in range(self.num_epoch):
            ...
            for step, item in enumerate(self.data_loader):
                ...
                # print(person_input.shape[0])

                # first, try batch_size = 1 and mini_batch = 1

                input_mask = torch.zeros((self.batch_size, max_len, 5), dtype=torch.long).to(device)
                ...
                for mask_len in range(1, person_traj_len[0]+1):  # from 1 -> len
                    ...
                    # print((person_label.size()),(input_mask.size()))
            ...
            with open('result.txt', 'ab') as f:
                np.savetxt('result.txt', acc_valid, delimiter=" ")
                np.savetxt('result.txt', acc_test, delimiter=" ")
            # task.close()
    ...
    # inference方法定义整个网络的结构，包括每一层的规格和连接顺序
    def inference(self):
        user_ids = []
        for t in range(self.num_epoch):
            # settings or validation and test
...
if __name__ == '__main__':
    ...
    [trajs, mat1, mat2s, mat2t, labels, lens, u_max, l_max, m_max, c_max] = file_data
    # print(trajs)
    # print(type(m_max))
    # print(mat2s[5, 5])
    ...
    stan = Model(t_dim=hours+1, l_dim=l_max+1, u_dim=u_max+1,  m_dim=m_max+1, c_dim=c_max+1, embed_dim=50, ex=ex, dropout=0.2)
