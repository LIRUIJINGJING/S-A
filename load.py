...
def euclidean(point, each):
    ...
    # print(np.sqrt((lon1 - lon2) ** 2 + (lat1 - lat2) ** 2))

    return np.sqrt((lon1 - lon2) ** 2 + (lat1 - lat2) ** 2)
    # eu = np.sqrt((lon1 - lon2) ** 2 + (lat1 - lat2) ** 2)
    # if(eu > 200):
    #     eu = 200
    # print(np.sqrt((lon1 - lon2) ** 2 + (lat1 - lat2) ** 2))
    # return eu
...
def rst_mat1(traj, poi):
    ...
    for i, item in enumerate(traj):
        for j, term in enumerate(traj):
            ...
            # print(haversine(lon1=poi_item[2], lat1=poi_item[1], lon2=poi_term[2], lat2=poi_term[1]))
            mat[i, j, 1] = abs(item[2] - term[2])
    return mat  # (*M, *M, [dis, tim])

# def cu_can(poi, l_max):
#     # poi(L, [l, lat, lon])
#     candidate_loc_pre = np.linspace(1, l_max, l_max)  # (L)
#     F = np.zeros((l_max, l_max))  # mat (L, L)
#     for i, loc1 in enumerate(candidate_loc_pre):
#         for j, loc2 in enumerate(candidate_loc_pre):
#             poi1, poi2 = poi[int(loc1) - 1], poi[int(loc2) - 1]  # retrieve poi by loc_id
#             F[i, j] = 0 if haversine(lon1=poi1[2], lat1=poi1[1], lon2=poi2[2], lat2=poi2[1]) > 7 else F[i, j] = 1
#     return F  # (L, L)
...
def rs_mat2s(poi, l_max):
    ...
    for i, loc1 in enumerate(candidate_loc ):
        ...
        for j, loc2 in enumerate(candidate_loc):
            poi1, poi2 = poi[int(loc1) - 1], poi[int(loc2) - 1]  # retrieve poi by loc_id
            # print(haversine(lon1=poi1[2], lat1=poi1[1], lon2=poi2[2], lat2=poi2[1]))
            mat[i, j] = haversine(lon1=poi1[2], lat1=poi1[1], lon2=poi2[2], lat2=poi2[1])
            # return mat if mat<20 else None
    return mat  # (L, L)
...
def process_traj(dname):  # start from 1
    ...
    # data = np.load('./data/' + dname + '.txt')
    poi = np.load('./data/' + dname + '_POI.npy')

    # oth = np.load('./data/' + dname + '_T&M.npy')


    num_user = data[-1, 0]  # max id of users, i.e. NUM
    data_user = data[:, 0]  # user_id sequence in data
    trajs, labels, mat1, mat2t, lens = [], [], [], [], []
    u_max, l_max, m_max, c_max = np.max(data[:, 0]), np.max(data[:, 1]), np.max(data[:, 3]), np.max(data[:, 4])
    print(type(l_max),type(m_max),type(c_max))
    ...
    for u_id in range(num_user+1):
        ...
        # print(np.where(data_user == u_id))
        user_traj = data[np.where(data_user == u_id)]  # find all check-ins of u_id
        # user_traj1 = oth[np.where(data_user == u_id)]  # find all check-ins of u_id
    ...
    data = [trajs, np.array(mat1), mat2s, np.array(mat2t), labels, np.array(lens), u_max, l_max, m_max, c_max]
